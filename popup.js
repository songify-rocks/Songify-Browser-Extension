document.addEventListener("DOMContentLoaded", () => {
  ...
  function requestVideoData() {
    ...
    chrome.storage.local.get(["currentVideoData"], (result) => {
      if (result.currentVideoData && result.currentVideoData.timestamp) {
        ...
        if (dataAge < 30000) {
          console.log(
            "Using stored video data from storage (age:",
            dataAge,
            "ms)"
          );
          ...
          console.log(
            "Stored video data is stale (age:",
            dataAge,
            "ms), requesting fresh data"
          );
        }
      } else {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (tabs[0] && tabs[0].url) {
        ...
        if (
          ...
          tabs[0].url.includes("music.youtube.com")
        ) {
  ...
  function updatePreviewSection(data) {
    ...
    if (data && data.originalTitle) {
      ...
      const isAmazonMusic =
        (data.source && data.source === "amazon_music") ||
      ...
      chrome.storage.local.get(["popupVideoData"], (result) => {
        if (result.popupVideoData) {
          ...
          originalTitleElement.textContent =
            result.popupVideoData.originalTitle || "Not available";
          filteredTitleElement.textContent =
            result.popupVideoData.filteredTitle || "Not available";
          channelNameElement.textContent =
            result.popupVideoData.channel || "Not available";
        } else {
  ...
  function updateConnectionStatus(isConnected, isValid = true) {
    ...
    statusText.textContent = isConnected
      ? "Connected to Songify"
      : "Not connected to Songify";
  }
  ...
  function addTerm() {
    ...
    chrome.storage.local.get(["filterTerms"], (result) => {
      ...
      chrome.storage.local.set({ filterTerms: terms }, () => {
        ...
        if (isSpecialPattern) {
          showStatus(
            `Special pattern added! This will remove all content inside ${term.charAt(
              0
            )}...${term.charAt(2)}`
          );
  ...
  function handleBracketWildcard() {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (
        ...
        showStatus(
          "Please navigate to a YouTube video or YouTube Music page",
          true
        );
      }
    });
      ...
      if (message.data) {
        ...
        chrome.storage.local.set(
          ...
          () => {
            ...
            if (
              document
                .getElementById("tab-now-playing")
  ...
  const videoDataInterval = setInterval(() => {
    if (
      document.getElementById("tab-now-playing").classList.contains("active")
    ) {
      requestVideoData();
    }
  }, 5000);
